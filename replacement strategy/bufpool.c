...
void removeFromUsedList(BufPool pool, int slot) {
    ...
    switch (pool->strategy) {
        ...
        case 'M':
            for (int k = 0; k < pool->nused; ++k) {
                if (pool->usedList[k] == slot) {
                    for (int l = k; l < pool->nused - 1; ++l) {
                        pool->usedList[l] = pool->usedList[l + 1];
                    }
                    pool->nused--;
                    break;
                }
    ...
    switch (pool->strategy) {
        ...
        case 'C':
           ...
           /* if (pool->nused == 0) {
            ...
            for (int i = 0; i < pool->nbufs; ++i) {
                //int s = (pool->curSlot + i) % (pool->nbufs);
                if (pool->bufs[pool->curSlot].pin == 0) {
                    slot = pool->curSlot;
                    pool->curSlot = ++pool->curSlot % pool->nbufs;
                    break;
                }
                pool->curSlot = ++pool->curSlot % pool->nbufs;
            }
            /*if (slot >= 0) {
                pool->curSlot = (slot + 1) % pool->nbufs;
            }*/
            //pool->nused--;
        ...
        case 'S':
            ...
            while (1) {
                ...
                pool->curSlot = pool->curSlot % pool->nbufs;
                struct buffer *b = &pool->bufs[pool->curSlot];
                if (b->pin == 0) {
                    if (b->usage == 0) {
                        slot = pool->curSlot;
                        pool->curSlot = (pool->curSlot + 1) % pool->nbufs;
                        break;
                    } else {
                        b->usage--;
                    }
...
void makeAvailable(BufPool pool, int slot) {
    switch (pool->strategy) {
        ...
        case 'M':
            ...
            for (int i = pool->nused; i > 0; --i) {
                temp = pool->usedList[i + 1];
                pool->usedList[i] = pool->usedList[i - 1];
            }
            pool->usedList[0] = slot;
            pool->nused++;
            // slot become most recently used
            break;
        case 'C':
...
void showPoolState(BufPool pool) {
    ...
    printf("%4s %6s %6s %6s %6s %6s\n", "Slot", "Page", "Pin", "usage", "Dirty", "NVB");
    for (i = 0; i < pool->nbufs; i++) {
        b = pool->bufs[i];
        p = pageInBuf(pool, i);
        printf("[%02d] %6s %6d %6d %6d", i, p, b.pin, b.usage, b.dirty);
        if (i == pool->curSlot) {
            printf("%6c\n", '*');
        } else {
            printf("\n");
